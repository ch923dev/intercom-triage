...
// Reply state — derived from the customer-visible thread. `internal_notes`
// (legacy Intercom renderable_type 2) are folded in: they are teammate replies,
// not internal notes. See TicketFlyout for the full explanation.
const adminReplyCount = computed(
  () => props.ticket.parts.filter((p) => p.is_admin).length + props.ticket.internal_notes.length,
);
...
<template>
  ...
  >
    ...
    <div
      v-if="followupLabel || noteLines || adminReplyCount || awaitingCustomer || isClosed"
      class="tags"
    >
