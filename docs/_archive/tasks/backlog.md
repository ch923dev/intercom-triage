...
- **T106 ✓ (implemented 2026-05-27, roadmap 4.1)** — Parked / snoozed state. Operator-chosen "waiting on customer / third party / internal" with a wake time + structured reason. Shipped: `parked_at` / `parked_until` / `parked_reason` columns (XOR-locked, never co-resolved; every resolve path clears them), park/unpark + bulk endpoints, **filter-chip** board placement (Layout B) with derived "ready to resume" (`parked_until ≤ now`, no scheduler), and an extension popup Parked tab. Spec: `FR-042` / `US-021`; design + plan under `docs/superpowers/`.
