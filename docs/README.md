...
the board behind OnlySales-delegated auth. The webapp is the sole client surface —
it gates the board behind login and refreshes its access token on 401.
...
health/metrics · **auth/users** · categories · proposals · tickets (read/ingest/**sync**) ·
tickets (single + bulk, capped 200 — incl. **assign**) · followups · notes · note entries ·
...
<details>
<summary><b>The 19 cross-package invariants</b> — the things that break if you forget them</summary>
...
auth guards every route except the allowlist (#15); sessions rotate with
reuse-detection (#16); attribution/assignment are board-state only (#17); no
password is ever stored (#19).

Index form: [`PROJECT.md` §7](./PROJECT.md#7-the-19-cross-package-invariants-index).
</details>
