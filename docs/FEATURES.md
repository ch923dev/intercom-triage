...
- **Severity-ranked Slack delivery, worst first** — bounded per pass (Slack allows ~1 msg/sec/channel). Post → store `ts` → mark delivered: a crash mid-way risks one duplicate post, the reverse order risks losing an alert permanently. (backend · `services/bug_alerts.py:deliver_pending_bug_alerts` · US-044/FR-077)
- **Evidence provenance verified, not trusted** — the quote must appear verbatim inside ONE customer-authored part, or it is dropped. `parts[]` legitimately carries admin replies, and live traffic caught the model quoting our own support agent describing the defect (1 in 14) — which reads in Slack as a customer report. Per-part containment rejects a quote stitched from two messages; case / whitespace / quote-and-dash typography are folded so a genuine quote is not lost to punctuation normalization. Dropping the quote never drops the verdict. (ai · `ai/pipeline.py:verify_bug_evidence` · US-044/FR-075a)
- **The card is a triage surface** — severity-coloured left rail (via a Slack attachment — Block Kit has no colour), the title linked to the conversation, the customer quote blockquoted, then reporter name / email / Intercom user id / phone / location / company, ticket state + age, owner, the AI summary, and a footer of category · confidence · occurrences · priority · sentiment · labels · ticket id. Every field is already denormalized on `tickets`, so this costs one wider `SELECT` and no second AI call; empty fields are omitted rather than rendered blank, and an alert whose ticket was deleted still posts. (backend · `services/bug_alerts.py:_alert_attachments` · US-044/FR-077a)
...
- **Read + dismiss API** — `GET /bug-alerts?severity=&delivered=` (worst first, includes `low` and dismissed rows on purpose — it is the calibration surface) and `POST /bug-alerts/{ticket_id}/dismiss` (idempotent; survives re-detection). Alert-state only: `BugAlertRead` is its own schema, never fields on `HydratedTicket` (inv #2). (backend · `routers/bug_alerts.py` · US-044/FR-079)
- **Bug review page (webapp)** — a Bugs tab listing alerts worst-first with the customer's quote, confidence, occurrence count, delivery state (announced / awaiting / dismissed, plus "escalation pending" while the verdict outranks what Slack was told), a Slack permalink when the pair is stored, and per-row Dismiss. Dismissal splices in the single row the endpoint returns rather than refetching, so a detection landing mid-action is not clobbered by a stale list. Nothing is hidden by default — `low` and dismissed rows stay listed, because this is the calibration surface. Loaded on first open, not at bootstrap. (webapp · `components/BugAlertsPage.vue`, `stores/bugAlerts.ts` · US-045/FR-080..FR-083)
- **Acknowledge, mirrored into Slack** — `POST /bug-alerts/{ticket_id}/ack` stamps `acked_at` + `acked_by` (the authenticated caller — never a client-supplied id) and then rewrites the alert's own Slack message via `chat.update` so the channel shows who owns it, with no second message posted. Acknowledgement is a state of its own, not a synonym for dismissal: acked = someone owns this, dismissed = finished, and a row may carry both. The pair is XOR-locked by CHECK and never written by the record pass, so it survives re-detection and escalation. Recorded before Slack is called: a Slack failure returns `slack_updated: false` and the acknowledgement stands (the board is the record; the channel is a projection). Idempotent — a second ack keeps the first operator and makes no Slack call. Acknowledgement travels **outward only**; nothing accepts a request from Slack, so there is no public endpoint, no signing secret, and no allowlist change (inv #15). (backend/webapp · `services/bug_alerts.py:ack_bug_alert`, `clients/slack.py:update_message`, `components/BugAlertsPage.vue`, migration 0028 · US-046/FR-084..FR-088)
- **Bug note (the incident record)** — `PUT /bug-alerts/{ticket_id}/note` stores what the defect turned out to be — root cause, workaround, what was done — with `note_by` (the MOST RECENT author; the board is team-wide so anyone may correct a note) and `note_at`. A CHECK-locked trio, ≤2000 chars, cleared by an empty body so "no note" has one representation. Durable operator knowledge: detection, delivery, ack, and dismissal never touch it, because it is absent from the record pass's explicit `ON CONFLICT` set-clause. (backend/webapp · `services/bug_alerts.py:set_bug_note`, migration 0029 · US-047/FR-089/FR-090)
- **Recurrence suggestion ("seen before")** — on announcing a bug, the card carries the most similar EARLIER noted bug and its note, so the prior fix is visible where triage starts; `GET /bug-alerts/{ticket_id}/similar` recomputes the same thing per request, so a note written after an announcement still reaches the next reader. Matching is symptom-to-symptom (the alert's evidence + the ticket's customer-visible text, never `internal_notes` — inv #4, and never the note itself, which would rank by remedy language). Deliberately **cross-category**, unlike `GET /playbooks/suggested`: the same defect arrives filed three different ways, and a category-scoped match misses exactly the recurrence this catches. A similarity floor rejects weak matches — silence beats a confidently-wrong prior fix. Embedding + cosine only: no AI call, cache key untouched (inv #6). Empty (never an error) when embeddings are off, which is the hosted-v1 default. (backend/webapp · `services/bug_alerts.py:similar_noted_bugs`, `ai/embeddings.py:cosine` · US-047/FR-091..FR-094)
