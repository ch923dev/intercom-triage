...
3. **`part_type` mapping (official API).** Intercom conversation parts carry a `part_type` string: `comment` (+ customer author → `parts[]`, admin/bot author → `parts[]` with `is_admin`), `note` → `internal_notes[]`, `open`/`close`/`snoozed`/auto reply-time notice/… → skipped. **`assignment` is conditional:** Intercom's "reply and assign" rides the admin's reply text on the `assignment` event, so an assignment *with a body* → `parts[]` (is_admin per author); a bodyless one is dropped by the empty-body guard (`_COMMENT_PART_TYPES = {comment, assignment}` in `intercom_normalizer.py`). The opening message lives on `source` and is emitted as the first part. Unknown `part_type` → skipped + logged (`intercom.unknown_part_type`). Stable + documented (replaces the old reverse-engineered numeric `renderable_type` codes).
